...
var DockerManager = class {
  ...
  async runCommand(command, options = {}) {
    return new Promise((resolve2, reject) => {
      const proc = (0, import_child_process.spawn)("docker-compose", command, {
        stdio: "inherit",
        ...options
      });
      let output = "";
...
program2.command("stop").description("Stop all ENS-IPFS services").action(async () => {
  try {
...
program2.command("status").description("Check the status of all services").action(async () => {
  try {
...
nodesCommand.command("add").description("Add a new node").argument("<name>", "Name of the node to add").argument("<url>", "URL of the node to add").action((name, url2) => {
  if (!name || !url2) {
    console.error("Both name and URL are required to add a new node.");
    return;
  }
  axios_default.post(base_api_url + "/nodes", new URLSearchParams({
    name,
    url: url2
  }).toString(), {
    headers: {
    ...
    let current_node = response.data["node"];
    console.log(
      current_node.id.padEnd(10) + current_node.name.padEnd(20) + current_node.url.padEnd(30) + current_node.type
    );
  }).catch((error) => {
    console.error("Error adding new node:", error.message);
  });
});
nodesCommand.command("count").description("Get the total count of all nodes").action(() => {
  axios_default.get(base_api_url + "/nodes/count").then((response) => {
    console.log(`Total nodes: ${response.data["count"]}`);
  }).catch((error) => {
    console.error("Error retrieving node count:", error.message);
  });
});
nodesCommand.command("delete").description("Remove a node by ID").argument("<id>", "Node ID to delete").action((id) => {
  axios_default.delete(base_api_url + "/nodes/" + id).then(
    (response) => {
